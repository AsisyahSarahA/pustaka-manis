        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div class="glass-panel rounded-4xl p-6">
                <h2 class="mb-4 text-lg font-bold text-pearl flex items-center gap-2">
                    <span class="flex h-7 w-7 items-center justify-center rounded-full bg-azure-soft/20 text-azure-soft text-xs">1</span>
                    Identifikasi Anggota
                </h2>

                {{-- STEP 1: Pilih Kategori Anggota (Tactile Skeuomorphic Buttons) --}}
                <div class="mb-5 space-y-2">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-pearl/70">
                        ...
                        <template x-for="cat in categories" :key="cat.id">
                            <button
                                ...
                                :class="selectedCategory === cat.id 
                                    ? 'bg-azure-soft text-navy-dark shadow-[inset_0_2px_4px_rgba(0,0,0,0.4)] border-azure-soft font-bold scale-[0.98]' 
                                    : 'bg-navy-soft/60 text-pearl border-white/10 hover:bg-white/10 hover:border-white/20 shadow-[0_4px_6px_rgba(0,0,0,0.3)]'"
                                class="flex flex-col items-center justify-center rounded-2xl border py-3 px-2 text-center transition-all duration-150 active:scale-95"
                            >
                                <span class="text-xl" x-text="cat.icon"></span>
                                <span class="mt-1 text-xs font-semibold" x-text="cat.label"></span>
                            </button>
                        </template>
                    </div>
                </div>

                {{-- STEP 2: Input / Scan Identitas (Hanya aktif jika kategori sudah dipilih) --}}
                <div class="space-y-3" :class="{ 'opacity-40 pointer-events-none': !selectedCategory }">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-pearl/70">
                        Step 2: Input / Scan Kartu (<span x-text="categoryLabel"></span>)
                    </label>
            ...
            <div class="glass-panel rounded-4xl p-6">
                <h2 class="mb-4 text-lg font-bold text-pearl flex items-center gap-2">
                    <span class="flex h-7 w-7 items-center justify-center rounded-full bg-azure-soft/20 text-azure-soft text-xs">2</span>
                    Scan Koleksi Buku
                </h2>
    ...
    <script>
        function borrowApp() {
            return {
                ...
                selectedCategory: '',
                categoryLabel: 'Pilih Kategori Terlebih Dahulu',
                member: null,
                ...
                async scanMember(manualCode = null) {
                    ...
                    if (!this.selectedCategory) {
                        this.memberError = 'Pilih kategori anggota terlebih dahulu!';
                        return;
                    }
